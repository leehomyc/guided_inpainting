...
class InpaintingDatasetApolloServerPredictSegmentationObjectRemoval(BaseDataset):
    def initialize(self, opt):
        ...
        if opt.isTrain:
            ...
            imagelistfile = open("{}/split/test.txt".format(self.root))
            for line in imagelistfile:
                self.paths.append(self.root + '/image/' + line.strip() + '.jpg')
                self.annpaths.append(self.root + '/label/' + line.strip() + '.png')
            imagelistfile.close()
    ...
    def name(self):
        return 'InpaintingDatasetApolloServerPredictSegmentationObjectRemoval'
