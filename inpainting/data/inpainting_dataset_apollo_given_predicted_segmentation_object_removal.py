...
class InpaintingDatasetApolloGivenPredictedSegmentationObjectRemoval(BaseDataset):
    def initialize(self, opt):
        self.opt = opt
        self.root = opt.dataroot

        files = ['_'.join(f.split('_')[0:4])+'_' for f in listdir(self.root) if isfile(join(self.root, f))]
    ...
    def name(self):
        return 'InpaintingDatasetApolloGivenPredictedSegmentationObjectRemoval'
