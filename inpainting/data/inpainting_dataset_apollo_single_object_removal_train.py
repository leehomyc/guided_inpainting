...
class InpaintingDatasetApolloSingleObjectRemovalTrain(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # random move the object
        random_x = np.random.randint(low = -bbox[2], high = self.opt.fineSize - bbox[3])
        random_y = np.random.randint(low = -bbox[0], high = self.opt.fineSize - bbox[1])
        bbox[2], bbox[3] = bbox[2]+random_x, bbox[3] + random_x
        bbox[0], bbox[1] = bbox[0]+random_y, bbox[1] + random_y
    ...
    def name(self):
        return 'InpaintingDatasetApolloSingleObjectRemovalTrain'
