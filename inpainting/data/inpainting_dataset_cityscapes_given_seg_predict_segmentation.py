...
class InpaintingDatasetCityscapesGivenSegPredictSegmentation(BaseDataset):
    def initialize(self, opt):
        self.opt = opt
        self.root = opt.dataroot

        files = ['_'.join(f.split('_')[0:3]) for f in listdir(self.root) if isfile(join(self.root, f))]

        filehash = set(files)
        fileid = [i for i in filehash]

        # self.paths = [self.root+'/'+i+'_origin.png' for i in fileid]
        self.paths = [self.root+'/'+i+'_leftImg8bit.png' for i in fileid]
        self.annpaths = [self.root+'/'+i+'_predLabel.png' for i in fileid]
    ...
    def name(self):
        return 'InpaintingDatasetCityscapesGivenSegPredictSegmentation'
