...
class InpaintingDatasetApolloTestPredictSegmentationObjectRemoval(BaseDataset):
    def initialize(self, opt):
        ...
        self.rootAnn = os.path.join(self.root,'label')
        self.rootImg = os.path.join(self.root,'rgb')

        # self.paths = sorted(make_dataset(self.rootImg))
        # self.annpaths = [ f.replace(self.rootImg, self.rootAnn).replace('.jpg','.png') for f in self.paths]
        self.annpaths = sorted(make_dataset(self.rootAnn))
        self.paths = [ f.replace(self.rootAnn, self.rootImg).replace('.png','.jpg') for f in self.annpaths]
    ...
    def __getitem__(self, index):
        ...
        if self.opt.isTrain:
            ...
            random_x = np.random.randint(low = int(padding_size/2), high = padding_size)
            random_y = np.random.randint(low = int(padding_size/2), high = padding_size)
    ...
    def name(self):
        return 'InpaintingDatasetApolloTestPredictSegmentationObjectRemoval'
